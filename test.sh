...
echo "== Starting tests..."
gocov test $TEST_PKGS -v -race -coverpkg $(join "," $PACKAGES) 1> /dev/null 2> >(grep -v "warning: no packages being tested depend on" >&2)
echo "== Building strict tests..."
go test -i -tags=strict -race $TEST_PKGS
echo "== Starting strict tests..."
gocov test $TEST_PKGS -v -tags=strict -race -coverpkg $(join "," $PACKAGES) 1> /dev/null 2> >(grep -v "warning: no packages being tested depend on" >&2)
echo "== Finished tests."